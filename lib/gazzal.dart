...
class GazzalPage extends StatefulWidget {
  const GazzalPage({super.key});

  @override
  State<GazzalPage> createState() => _GazzalPageState();
}

class _GazzalPageState extends State<GazzalPage> {
  ...
  Widget scrollImage(BuildContext context, String category) {
    final names = {
      ...
      'Desert': ['Ice Cream', 'Sweets', 'Pudding', 'Brownies', 'Donuts'],
    };
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return _buildEventCard(
                  'assets/guzzle/g1/${category.toLowerCase()}${index + 1}.png',
  ...
  Widget _buildEventCard(
    ...
    return GestureDetector(
      onTap: () {
        if (category == 'Theme' || category == 'Food'|| category == 'Beverage'|| category == 'Desert'|| category == 'Music') {
          showDialog(
            context: context,
            builder: (context) {
              return Dialog(
      ...
      for (var i = 0; i < _selectedImages[category]!.length; i++) {
        if (_selectedImages[category]![i]) {
          selectedItems.add({
            ...
            'imagePath': 'assets/guzzle/g1/${category.toLowerCase()}${i + 1}.png',
          });
